...
SingularValueDecomposition::SingularValueDecomposition(const Mat* pmArg, Matd* /*pmS*//*=NULL*/, Matd* /*pmU*//*=NULL*/, Matd* /*pmV*//*=NULL*/)
{
	...
	U = new double*[m]; U[0] = new double[m*nu]; memset (U[0], 0, sizeof(double) * m*nu);
	for (int i = 1; i < m; i++) U[i] = &U[0][nu*i];

	V = new double*[n]; V[0] = new double[n*n]; memset (V[0], 0, sizeof(double) * n*n);
	for (int i2 = 1; i2 < n; i2++) V[i2] = &V[0][n*i2];
...
const double* SingularValueDecomposition::singularValues()
{
	return s;
}
...
void svd(const Mat& m, Mat& u, Mat& d, Mat& v)
{
	SingularValueDecomposition svdOp(&m);
	Mat* pmu = svdOp.GetU();
	u = *pmu;
	delete pmu;
