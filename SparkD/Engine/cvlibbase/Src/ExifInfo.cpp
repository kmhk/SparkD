...
////////////////////////////////////////////////////////////////////////////////
bool CxExifInfo::decodeExif(XFile * hFile, int nReadMode)
{
    int a;
    bool HaveCom = false;
        ...
        switch(marker){
            ...
            case M_EXIF:
                ...
                if ((nReadMode & EXIF_READ_EXIF) && memcmp(data+2, "Exif", 4) == 0){
                    m_exifinfo->IsExif = process_EXIF((uchar *)data+2, itemlen-2);
                }else{
                    // Discard this section.
                    free(Sections[--SectionsRead].data);
					Sections[SectionsRead].data=0;
...
--------------------------------------------------------------------------*/
bool CxExifInfo::process_EXIF(uchar * CharBuf, uint length)
{
...
--------------------------------------------------------------------------*/
int CxExifInfo::Get32s(void * Long)
{
    if (MotorolaOrder){
...
--------------------------------------------------------------------------*/
uint CxExifInfo::Get32u(void * Long)
{
    return (uint)Get32s(Long) & 0xffffffff;
}
////////////////////////////////////////////////////////////////////////////////
...
--------------------------------------------------------------------------*/
bool CxExifInfo::ProcessExifDir(uchar * DirStart, uchar * OffsetBase, unsigned ExifLength,
                           ExifInfo * const exifinfo, uchar ** const LastExifRefdP, int NestingLevel)
{
    int de;
    int a;
    int NumDirEntries;
    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        switch(Tag){
            ...
            case TAG_MAKE:
                strncpy(exifinfo->CameraMake, (char*)ValuePtr, 31);
                break;

            case TAG_MODEL:
                strncpy(exifinfo->CameraModel, (char*)ValuePtr, 39);
                break;

			case TAG_EXIF_VERSION:
				strncpy(exifinfo->Version,(char*)ValuePtr, 4);
				break;
            ...
            case TAG_USERCOMMENT:
                ...
                if (memcmp(ValuePtr, "ASCII",5) == 0){
                    for (a=5;a<10;a++){
                        ...
                        c = ((char*)ValuePtr)[a];
                        if (c != '\0' && c != ' '){
                            strncpy(exifinfo->Comments, (char*)ValuePtr+a, 199);
                            break;
                        }
                    }
                    
                }else{
                    strncpy(exifinfo->Comments, (char*)ValuePtr, 199);
                }
                break;
            ...
            case TAG_FNUMBER:
                /* Simplest way of expressing aperture, so I trust it the most.
                   ...
                   */
				exifinfo->ApertureFNumber = (float)ConvertAnyFormat(ValuePtr, format);
                break;
            ...
            case TAG_MAXAPERTURE:
                ...
                */
                if (exifinfo->ApertureFNumber == 0){
					exifinfo->ApertureFNumber = (float)exp(ConvertAnyFormat(ValuePtr, format)*log(2.0f)*0.5);
                }
                break;

			case TAG_BRIGHTNESS:
				exifinfo->Brightness = (float)ConvertAnyFormat(ValuePtr, format);
				break;
            ...
            case TAG_FOCALLENGTH:
                ...
                */
				...
				exifinfo->FocalLength = (float)ConvertAnyFormat(ValuePtr, format);
                break;
            ...
            case TAG_SUBJECT_DISTANCE:
                ...
                */
				exifinfo->Distance = (float)ConvertAnyFormat(ValuePtr, format);
                break;
            ...
            case TAG_ORIENTATION:
				exifinfo->Orientation = (int)ConvertAnyFormat(ValuePtr, format);
            ...
            case TAG_FOCALPLANEXRES:
				exifinfo->FocalplaneXRes = (float)ConvertAnyFormat(ValuePtr, format);
                break;

            case TAG_FOCALPLANEYRES:
				exifinfo->FocalplaneYRes = (float)ConvertAnyFormat(ValuePtr, format);
                break;

			case TAG_RESOLUTIONUNIT:
                switch((int)ConvertAnyFormat(ValuePtr, format)){
                    case 1: exifinfo->ResolutionUnit = 1.0f; break; /* 1 inch */
                    case 2:	exifinfo->ResolutionUnit = 1.0f; break;
                    case 3: exifinfo->ResolutionUnit = 0.3937007874f;    break;  /* 1 centimeter*/
                    case 4: exifinfo->ResolutionUnit = 0.03937007874f;   break;  /* 1 millimeter*/
                    case 5: exifinfo->ResolutionUnit = 0.00003937007874f;  /* 1 micrometer*/
                }
                break;

            case TAG_FOCALPLANEUNITS:
                switch((int)ConvertAnyFormat(ValuePtr, format)){
                    case 1: exifinfo->FocalplaneUnits = 1.0f; break; /* 1 inch */
                    case 2:	exifinfo->FocalplaneUnits = 1.0f; break;
                    case 3: exifinfo->FocalplaneUnits = 0.3937007874f;    break;  /* 1 centimeter*/
                    case 4: exifinfo->FocalplaneUnits = 0.03937007874f;   break;  /* 1 millimeter*/
                    case 5: exifinfo->FocalplaneUnits = 0.00003937007874f;  /* 1 micrometer*/
                }
                break;
            ...
            case TAG_EXPOSURE_BIAS:
				exifinfo->ExposureBias = (float) ConvertAnyFormat(ValuePtr, format);
                break;

            case TAG_WHITEBALANCE:
				exifinfo->Whitebalance = (int)ConvertAnyFormat(ValuePtr, format);
                break;

            case TAG_METERING_MODE:
				exifinfo->MeteringMode = (int)ConvertAnyFormat(ValuePtr, format);
                break;

            case TAG_EXPOSURE_PROGRAM:
				exifinfo->ExposureProgram = (int)ConvertAnyFormat(ValuePtr, format);
                break;

            case TAG_ISO_EQUIVALENT:
				exifinfo->ISOequivalent = (int)ConvertAnyFormat(ValuePtr, format);
                if (exifinfo->ISOequivalent < 50 ) exifinfo->ISOequivalent *= 200;
                break;

            case TAG_COMPRESSION_LEVEL:
				exifinfo->CompressionLevel = (int)ConvertAnyFormat(ValuePtr, format);
                break;

            case TAG_XRESOLUTION:
				exifinfo->Xresolution = (float)ConvertAnyFormat(ValuePtr, format);
                break;
            case TAG_YRESOLUTION:
				exifinfo->Yresolution = (float)ConvertAnyFormat(ValuePtr, format);
                break;
        ...
        if (Tag == TAG_EXIF_OFFSET || Tag == TAG_INTEROP_OFFSET){
			...
			if (Offset>8){
				...
				ProcessExifDir(SubdirStart, OffsetBase, ExifLength, exifinfo, LastExifRefdP, NestingLevel+1);
			}
            continue;
        }
    }
    ...
    if (ThumbnailSize && ThumbnailOffset){
        if (ThumbnailSize + ThumbnailOffset <= ExifLength){
			...
			exifinfo->ThumbnailPointer = OffsetBase + ThumbnailOffset;
...
double CxExifInfo::ConvertAnyFormat(void * ValuePtr, int format)
{
    ...
    switch(format){
        ...
        case FMT_SSHORT:    value = (short)Get16u(ValuePtr);  break;
...
////////////////////////////////////////////////////////////////////////////////
void CxExifInfo::process_COM (const uchar * data, int length)
{
    int ch;
...
////////////////////////////////////////////////////////////////////////////////
void CxExifInfo::process_SOFn (const uchar * data, int marker)
{
...
////////////////////////////////////////////////////////////////////////////////
bool CxExifInfo::encodeExif(XFile * hFile)
{
    int a;
    ...
    for (a=0;a<SectionsRead-1;a++){
        hFile->putC(0xff);
        hFile->putC((uchar)(Sections[a].type));
...
////////////////////////////////////////////////////////////////////////////////
bool loadEXIF(const char* szfilename, ExifInfo& exifinfo)
{
	...
	if (!fp)
		return false;
	XFileDisk file(fp);
	CxExifInfo cxexifinfo(&exifinfo);
	bool fread=cxexifinfo.decodeExif (&file);
	fclose(fp);
	return fread;
}
