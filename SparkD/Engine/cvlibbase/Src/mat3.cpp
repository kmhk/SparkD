...
mat3::mat3(nv_scalar v)
{
    memset(mat_array, 0, sizeof(mat_array));
	a00=a11=a22=v;
}
...
void mat4::set_rot(const quat& q_)
{
	mat3 m;
	q_.toMatrix(m);
	set_rot(m);
...
void mat4::set_rot(const vec3& u, const vec3& v)
{
    ...
    nv_scalar hxy = w_.x * w_.y * h;
    nv_scalar hxz = w_.x * w_.z * h;
    nv_scalar hyz = w_.y * w_.z * h;
...
quat & mat4::get_rot(quat& q_) const
{
	mat3 m;
	get_rot(m);
	q_.FromMatrix(m);
...
{
    vec3 cp;
    vec3 e0(v1.x - v0.x, t1.x - t0.x, t1.y - t0.y);
    vec3 e1(v2.x - v0.x, t2.x - t0.x, t2.y - t0.y);
