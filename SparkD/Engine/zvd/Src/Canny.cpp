...
{
	assert (pmIn->type() == MAT_Tuchar);

	int nWidth = pmIn->cols();
	int	nHeight = pmIn->rows();
	uchar**	ppbIn = pmIn->data.ptr;
	uchar**	ppbOut = pmOut ? pmOut->data.ptr: ppbIn;
	...
	memset(ppbOut[0], 0, sizeof(uchar) * nHeight * nWidth);
...
{
	assert (pmIn->type1()==MAT_Tuchar3);
	assert (pmOut->type1()==MAT_Tuchar);
	int nWidth = pmIn->cols();
	int	nHeight = pmIn->rows();
	uchar**	ppbIn = pmIn->data.ptr;
	uchar**	ppbOut = pmOut ? pmOut->data.ptr: ppbIn;
...
void canny (const Mat& src, Mat& dst, float low_thresh, float high_thresh, int mode)
{
	...
	uchar**	ppbIn = src.data.ptr;

	int			iH, iW;
	int			high = (int)high_thresh, low = (int)low_thresh;
	dst.create (nHeight, nWidth, MAT_Tuchar);
	memset(dst.data.ptr[0], 0, sizeof(uchar) * nHeight * nWidth);
...
void canny (Mat& src, float low_thresh, float high_thresh, int mode)
{
	Mat t=src;
	canny (t, src, low_thresh, high_thresh, mode);
}
