...
void	Sobel::process(Mat* pIn, Mat* pOut /* = NULL */)
{
	...
	{
		matTemp.convertTo (*pOut, MAT_Tuchar);
	}
}
...
{
	assert (pmatIn->type() == MAT_Tuchar);
//	assert (pmatOut->type() == MAT_Tuchar);
...
{
	out.create (in.rows()-2,in.cols()-2,MAT_Tuchar);
	cvlib::Sobel sobel;
...
//            kernel *= scale;
    ...
    else
    {
       /* const size_t STRIPE_SIZE = 1 << 14;
        ...
        int wdepth = depth == MAT_Tuchar && ksize <= 5 ? MAT_Tshort : depth <= MAT_Tfloat ? MAT_Tfloat : MAT_Tdouble;
