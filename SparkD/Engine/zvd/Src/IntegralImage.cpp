...
public:
	...
	void	process (const Mat* pmImg, Mat* pmSum, Mat* pmSquare = NULL);
	void	ProcessEx (const Mat* pmImg, Mat* pmSum, Mat* pmTilted, Mat* pmSquare = NULL);

protected:
	void	iProcess_Byte (const Mat* pmImg, Mat* pmSum, Mat* pmSquare = NULL);
	void	iProcess_int (const Mat* pmImg, Mat* pmSum, Mat* pmSquare = NULL);
	void	iProcess_float (const Mat* pmImg, Mat* pmSum, Mat* pmSquare = NULL);
};


void	IntegralImage::process (const Mat* pmImg, Mat* pmSum, Mat* pmSquare/* = NULL */)
{
	switch (pmImg->type())
	{
	case MAT_Tuchar:
...
void	IntegralImage::iProcess_Byte (const Mat* pmImg, Mat* pmSum, Mat* pmSquare/* = NULL */)
{
#define worktype int

	assert (pmImg->type() == MAT_Tuchar);
	if (pmSum)
...
void	IntegralImage::iProcess_int (const Mat* pmImg, Mat* pmSum, Mat* pmSquare/* = NULL */)
{
#define worktype int
...
void	IntegralImage::iProcess_float(const Mat* pmImg, Mat* pmSum, Mat* pmSquare/* = NULL */)
{
#undef worktype
		...
		else
		{
			...
			{ 
				...
				for( x = 0, s = 0, sq = 0; x < size.width; x++ )
				{
					...
					s += it;
					sq += (double)it * it;
					double t = (float)(mSum[x - sumstep] + s);
...
void	IntegralImage::ProcessEx (const Mat* pmImg, Mat* pmSum, Mat* pmTilted, Mat* pmSquare/* = NULL*/)
{
#undef worktype
#define worktype int
	...
	double* sqsum = pmSquare ? pmSquare->data.db[0]: NULL;
	int srcstep, sumstep=0, tiltedstep=0, sqsumstep=0;
	Size size = pmImg->size ();
	srcstep = size.width;
...
void integralImage(const Mat& src, Mat& sum)
{
	if (src.type()==MAT_Tuchar || src.type()==MAT_Tint)
		sum.create (src.rows()+1, src.cols()+1, MAT_Tint);	
	else
		sum.create (src.rows()+1, src.cols()+1, MAT_Tdouble);	
	IntegralImage integ;
	integ.process (&src, &sum);
}
void integralImage(const Mat& src, Mat& sum, Mat& square)
{
	if (src.type()==MAT_Tuchar || src.type()==MAT_Tint)
	{
		sum.create (src.rows()+1, src.cols()+1, MAT_Tint);
		square.create (src.rows()+1, src.cols()+1, MAT_Tdouble);
	}
	else
	{
		sum.create (src.rows()+1, src.cols()+1, MAT_Tdouble);	
	...
	IntegralImage integ;
	integ.process (&src, &sum, &square);
}
void integralImage(const Mat& src, Mat& sum, Mat& tilted, Mat& square)
{
	if (src.type()==MAT_Tuchar || src.type()==MAT_Tint)
	{
		sum.create (src.rows()+1, src.cols()+1, MAT_Tint);
		tilted.create (src.rows()+1, src.cols()+1, MAT_Tint);
		square.create (src.rows()+1, src.cols()+1, MAT_Tdouble);
	}
	else
	{
		sum.create (src.rows()+1, src.cols()+1, MAT_Tdouble);
		tilted.create (src.rows()+1, src.cols()+1, MAT_Tdouble);
		square.create (src.rows()+1, src.cols()+1, MAT_Tdouble);
	}

	IntegralImage integ;
	integ.ProcessEx (&src, &sum, &tilted, &square);
}
void integralTiltedImage(const Mat& src, Mat& sum, Mat& tilted)
{
	if (src.type()==MAT_Tuchar || src.type()==MAT_Tint)
	{
		sum.create (src.rows()+1, src.cols()+1, MAT_Tint);
	...
	IntegralImage integ;
	integ.ProcessEx (&src, &sum, &tilted, 0);
}
double sumIntegralImage(const Mat& sum, const Rect& region)
{
	double rsum=0;
	if (sum.type() == MAT_Tint)
	{
		int** ppnsum=sum.data.i;
		rsum=ppnsum[region.y][region.x]+ppnsum[region.limy()][region.limx()]-
			ppnsum[region.y][region.limx()]-ppnsum[region.limy()][region.x];
	}
	else if (sum.type()==MAT_Tdouble)
	{
		double** ppnsum=sum.data.db;
		rsum=ppnsum[region.y][region.x]+ppnsum[region.limy()][region.limx()]-
			ppnsum[region.y][region.limx()]-ppnsum[region.limy()][region.x];
	}
	return rsum;
}
void integralSquareImage(const Mat& src, Mat& square)
{
	square.create (src.rows()+1, src.cols()+1, MAT_Tdouble);
	IntegralImage integ;
	integ.ProcessEx (&src, 0, 0, &square);
}
